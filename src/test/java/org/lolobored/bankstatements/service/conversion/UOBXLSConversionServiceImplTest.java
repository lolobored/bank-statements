...
class UOBXLSConversionServiceImplTest {
    ...
    private static final SimpleDateFormat UOB_DATE_FORMAT = new SimpleDateFormat("dd MMM yyyy", Locale.ENGLISH);
    ...
    void convertsDebitTransaction() throws ParseException, IOException {
        File xls = createXls(new String[][]{
                {"01 Mar 2024", "UBER EATS", "12.99", "0.0"}
        });

        Statement statement = service.convertTableToTransactions("SG456", Statement.DEBIT_ACCOUNT, xls.getAbsolutePath());
    ...
    void skipsRowsBeforeTransactionDateHeader() throws ParseException, IOException {
        // The createXls helper puts a preamble row before the header — verify it's not parsed as a transaction
        File xls = createXls(new String[][]{
                {"01 Mar 2024", "ONLY TX", "10.0", "0.0"}
        });
