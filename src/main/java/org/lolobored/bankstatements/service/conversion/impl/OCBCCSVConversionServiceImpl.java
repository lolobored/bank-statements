...
public class OCBCCSVConversionServiceImpl implements OCBCCSVConversionService {
  ...
  public Statement convertTableToTransactions(String accountNumber, String accountType, String csv)
    ...
    for (OCBCCSVLine ocbccsvLine : ocbccsvLines) {
      ...
      transaction.setDate(LocalDate.parse(ocbccsvLine.getValueDate(), DATE_FORMATTER));
      BigDecimal amount = BigDecimal.ZERO;
      if (!ocbccsvLine.getDebitAmount().isEmpty()) {
        amount =
            BigDecimal.valueOf(
                Double.parseDouble("-" + ocbccsvLine.getDebitAmount().replace(",", "")));
      } else {
        amount =
            BigDecimal.valueOf(Double.parseDouble(ocbccsvLine.getCreditAmount().replace(",", "")));
      }
  ...
  private String getDescription(String description) {
    ...
    // remove the S followed by date at the end of the description as it seems to create issues when
    // reconciling
    description = description.replaceAll("[0-9]{2}/[0-9]{2}/[0-9]{2}", "");
