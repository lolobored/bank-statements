...
public class OCBCCreditTransactionsPage {
  ...
  public void downloadCsv() throws InterruptedException {
    ...
    logger.debug(
        "[TIMING] OCBCCredit: initial render sleep (budget {}ms): {}ms",
        RENDER_PAUSE_MS,
    ...
    logger.debug(
        "[TIMING] OCBCCredit: wait for period dropdown clickable: {}ms",
        System.currentTimeMillis() - t0);
    ...
    logger.debug(
        "[TIMING] OCBCCredit: wait for PAST_30_DAYS option visible: {}ms",
        System.currentTimeMillis() - t0);
    ...
    logger.debug(
        "[TIMING] OCBCCredit: wait for loading backdrop gone: {}ms",
        System.currentTimeMillis() - t0);

    t0 = System.currentTimeMillis();
    wait.until(ExpectedConditions.elementToBeClickable(DOWNLOAD_BUTTON));
    logger.debug(
        "[TIMING] OCBCCredit: wait for download button clickable: {}ms",
        System.currentTimeMillis() - t0);
    ...
    logger.debug(
        "[TIMING] OCBCCredit: wait for CSV option visible: {}ms", System.currentTimeMillis() - t0);
