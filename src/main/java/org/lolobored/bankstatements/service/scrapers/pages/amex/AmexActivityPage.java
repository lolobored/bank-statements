...
public class AmexActivityPage {
  ...
  private static final By DOWNLOAD_MODAL =
      By.className("axp-activity-download__DownloadModal__downloadModal___2WSh8");
  private static final By CSV_RADIO =
      By.id("axp-activity-download-body-selection-options-type_csv");
  private static final By INCLUDE_ALL =
      By.id("axp-activity-download-body-checkbox-options-includeAll");
  private static final By CONFIRM_BUTTON = By.className("btn-primary");
  private static final By ACCOUNT_NAME =
      By.className("axp-account-switcher__accountSwitcher__lastFive___1s6L_");
  ...
  /**
   * Clicks "View By Year", stores the parent element, and returns the number of year tabs. Must be
   * called before {@link #downloadCsvForYearIndex(int)}.
   */
  ...
  /**
   * Refreshes the year link list (DOM updates after each click), selects by index, then downloads
   * via the modal dialog.
   */
