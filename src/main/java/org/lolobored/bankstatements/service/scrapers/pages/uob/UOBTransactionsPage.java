...
public class UOBTransactionsPage {
  ...
  private void downloadForPeriod(String period) throws InterruptedException {
    ...
    logger.debug(
        "[TIMING] UOBTransactions({}): wait for frequency dropdown visible: {}ms",
        period,
    ...
    logger.debug(
        "[TIMING] UOBTransactions({}): post-dropdown-visible sleep (budget {}ms): {}ms",
        period,
        RENDER_PAUSE_MS,
        System.currentTimeMillis() - t0);
    ...
    logger.debug(
        "[TIMING] UOBTransactions({}): post-period-select sleep (budget {}ms): {}ms",
        period,
        PERIOD_SETTLE_MS,
        System.currentTimeMillis() - t0);
    ...
    logger.debug(
        "[TIMING] UOBTransactions({}): wait for download icon visible: {}ms",
        period,
        System.currentTimeMillis() - t0);
